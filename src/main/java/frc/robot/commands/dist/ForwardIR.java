...
public class ForwardIR extends CommandBase {
    ...
    public ForwardIR(double setpoint, double epsilon, boolean use_Zpid,int num) {
    ...
    void checkAndCorrectStuckEncoder(double currentEncoderValue) {
        if (areWithinRange(currentEncoderValue, lastEncoderValue) && !pidYAxis.atSetpoint() && pidYAxis.getI() != INTEGRAL_ENABLED_I) {
            encoderStuckCounter++;
